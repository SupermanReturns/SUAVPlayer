...
@property (nonatomic, strong) SForwardView *forwardView;
...
- (void)setUpTopView{
    SPlayerViewTopView *topView = [[SPlayerViewTopView alloc] init];
...
- (void)setupBottomView{
    SPlayerViewBottomView *bottomView = [[SPlayerViewBottomView alloc] init];
    ...
    if (gesture.state == UIGestureRecognizerStateChanged )
    {
        ...
        switch (self.direction) {
            ...
            case PanDirectionRight: // 向右划(快进)
            {
                ...
                self.forwardView.mode = SForwardViewModeForward; // 快进模式
                ...
                if (panLength > 0) {
                    ...
                    // 否则开始向左划 快退显示
                    self.forwardView.mode = SForwardViewModeRewind;
                    CGPoint endPoint = [gesture locationInView:gesture.view];
                    int panLength = endPoint.x - self.startPoint.x;
            ...
            case PanDirectionLeft: // 向左划 快退手势
            {
                ...
                if (panLength <  0) {
                    ...
                    self.forwardView.mode = SForwardViewModeForward;
...
- (SForwardView *)forwardView{
    if (!_forwardView) {
        _forwardView = [[SForwardView alloc] init];
