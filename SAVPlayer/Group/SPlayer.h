...
@protocol SPlayerDelegate <NSObject>

// 播放完成
- (void)playerPlaybackDidEnd:(SPlayer *)player;
/**
 *  播放状态发发生改变
 *
 *  @param player player
 *  @param state  播放状态
 */
- (void)player:(SPlayer *)player playStateChanged:(PlayerPlaybackState)state;
/**
 *  监听屏幕方向的改变
 *
 *  @param player      player
 ...
 */
- (void)player:(SPlayer *)player orientationChanged:(UIDeviceOrientation)orientation;
@end

@interface SPlayer : UIView
/**
 *  根据frame 初始化一个播放器
 *
 *  @param frame       frame
...
@property (nonatomic, weak) id <SPlayerDelegate> delegate;
