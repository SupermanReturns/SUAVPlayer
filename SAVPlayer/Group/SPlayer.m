...
static NSString * const SPlayerTracksKey = @"tracks";
static NSString * const SPlayerPlayableKey = @"playable";
static NSString * const SPlayerDurationKey = @"duration";


// KVO Player Keys
static NSString * const SPlayerRateKey = @"rate";  // rate
// KVO Player Item Keys
static NSString * const SPlayerStatusKey = @"status"; // 状态
static NSString * const SPlayerEmptyBufferKey = @"playbackBufferEmpty"; // 缓冲操作
static NSString * const SPlayerPlayerKeepUpKey = @"playbackLikelyToKeepUp";
static NSString * const SPlayerPlayerBufferFullKey = @"playbackBufferFull";
// KVO Player Preload Keys
static NSString * const SPlayerPlayerLoadedTimeRanges = @"loadedTimeRanges";  // 缓冲进度

// KVO Contexts
static NSString * const SPlayerObserverContext = @"SPlayerObserverContext";
static NSString * const SPlayerItemObserverContext = @"SPlayerItemObserverContext";
static NSString * const SPlayerPreloadObserverContext = @"SPlayerPreloadObserverContext";
@interface SPlayer()<SPlayerViewBottomViewDelegate,SPlayerViewDelegate>{
@private
 ...
 *  播放器的view 提供各种操作
 */
@property (nonatomic, weak) SPlayerView *playerView;
...
- (void)setupPlayerView {
    ...
    if (!_playerView) {
        SPlayerView *playerView = [[SPlayerView alloc] init];
        playerView.delegate = self;
        playerView.bottomView.delegate = self;
        [self setPlayerView:playerView];
    }
}
- (void)setPlayerView:(SPlayerView *)playerView {
    if (_playerView) {
...
- (void)addObserverWithPlayer:(AVPlayer *)player {
    ...
    [player addObserver:self forKeyPath:SPlayerRateKey options:(NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:(__bridge void *)SPlayerObserverContext];
    // Player Observer
...
- (void)removeObserverWithPlayer:(AVPlayer *)player {
    [player removeObserver:self forKeyPath:SPlayerRateKey context:(__bridge void *)SPlayerObserverContext];
...
- (void)removeObserverWithPlayerItem:(AVPlayerItem *)playerItem {
    [playerItem removeObserver:self forKeyPath:SPlayerEmptyBufferKey context:(__bridge void *)SPlayerItemObserverContext];
    [playerItem removeObserver:self forKeyPath:SPlayerPlayerKeepUpKey context:(__bridge void *)SPlayerItemObserverContext];
    [playerItem removeObserver:self forKeyPath:SPlayerPlayerBufferFullKey context:(__bridge void *)SPlayerItemObserverContext];
    [playerItem removeObserver:self forKeyPath:SPlayerStatusKey context:(__bridge void *)SPlayerItemObserverContext];
    [playerItem removeObserver:self forKeyPath:SPlayerPlayerLoadedTimeRanges context:(__bridge void *)SPlayerPreloadObserverContext];
}
...
- (void)addObserverWithPlayerItem:(AVPlayerItem *)playerItem {
    ...
    [playerItem addObserver:self forKeyPath:SPlayerStatusKey options:(NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:(__bridge void *)(SPlayerItemObserverContext)];
    [playerItem addObserver:self forKeyPath:SPlayerPlayerLoadedTimeRanges options:(NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:(__bridge void *)(SPlayerPreloadObserverContext)];
    [playerItem addObserver:self forKeyPath:SPlayerEmptyBufferKey options:(NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:(__bridge void *)(SPlayerItemObserverContext)];
    [playerItem addObserver:self forKeyPath:SPlayerPlayerKeepUpKey options:(NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:(__bridge void *)(SPlayerItemObserverContext)];
    [playerItem addObserver:self forKeyPath:SPlayerPlayerBufferFullKey options:(NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:(__bridge void *)(SPlayerItemObserverContext)];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context{
    ...
    if (context == (__bridge void *)(SPlayerObserverContext)) {
        // Player KVO
        if ([keyPath isEqualToString:SPlayerRateKey]) {
    ...
    } else if (context == (__bridge void *)(SPlayerItemObserverContext)) {
        // PlayerItem KVO
        if ([keyPath isEqualToString:SPlayerEmptyBufferKey]) {
            if (self.playerItem.playbackBufferEmpty) {
                _userPause = NO;
                self.bufferingState = PlayerBufferingStateDelayed;
            }
        } else if ([keyPath isEqualToString:SPlayerPlayerKeepUpKey]) {
            if (self.playerItem.playbackLikelyToKeepUp) {
                
                self.bufferingState = PlayerBufferingStateKeepUp;
            }
        } else if ([keyPath isEqualToString:SPlayerPlayerBufferFullKey]) {
            if (self.playerItem.playbackBufferFull) {
                self.bufferingState = PlayerBufferingStateFull;
            }
        }else if ([keyPath isEqualToString:SPlayerStatusKey]) {
    ...
    } else if (context == (__bridge void *)SPlayerPreloadObserverContext) {
        if ([keyPath isEqualToString:SPlayerPlayerLoadedTimeRanges]) {
